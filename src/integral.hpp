...
/// partition of the interval of integration into subintervals of equal length.
/// As integral() executes, however, each of these is adaptively subdivided as
...
struct integration_subinterval_stack
   ...
   /// interval of the variable of integration.  The integrand is evaluated at
   /// a minimum of two points (the end points).  The n-1 corresponding
...
///
/// NOTE: If the function itself call integral(), as for a double, a triple, or
/// a higher-level integral, then the tolerance specified for the innermost
/// integral must be the tightest, and the ratio of the tolerance between any
/// integral and the integral at the next level out must be at most about 0.05.
/// So, for example, a double integral with tolerance 1.0E-03 should have the
/// outer tolerance set to 1.0E-03 and the inner tolerance set at most to
/// 5.0E-05.  Otherwise, the outer integral will fruitlessly make too many
/// subdivisions and consume too much time, possibly never to converge.
///
/// The specified tolerance is on the relative accuracy of the estimated mean
/// value of the function over each subdivided interval of the variable of
/// integration.  Every interval is recursively subdivided into equal halves.
/// At each stage in the subdivision, the estimated mean height of the function
/// over the undivided interval is compared to the refined estimate after
/// subdivision of the interval.  If the magnitude of the difference between
/// these estimates be smaller than the product of the tolerance and the
/// refined estimate, then recursion at the current interval ceases.
///
/// Recursion is not done by way of the function-call stack but rather by way
/// of a stack with dynamically allocated memory.
///
...
/// Vaughan, who wrote it in 2016.  The meaning of the tolerance is different
/// in Vaughan's implementation, which also adds a test for the numerical
/// limitation of double-precision numbers.
///
...
auto integral(F f, A a, A b, double t = 1.0E-06, unsigned n = 16)
      // See Page 28 of Effective Modern C++ by Scott Meyers.
      -> decltype(std::forward<F>(f)(a) * a)
{
   using R = decltype(std::forward<F>(f)(a));
   using I = decltype(std::forward<F>(f)(a) * a);
   ...
   while (s.size()) {
      ...
      // The trapezoidal rule gives len*mean.  The composite trapezoidal rule
      // gives 0.5*len*(mean + fmid).  The first is the current estimate for
      // the current subinterval.  The second is the refined estimate obtained
      // by subdividing the current subinterval.
      //
      // The magnitude of the difference between the current estimate and the
      // refined estimate is u1.  We are done refining our estimate when u1
      // become smaller than a certain fraction of the magnitude of the refined
      // estimate.  That fraction is just the specified tolerance t.
      R const u1 = fabs(mean - rmean);
      R const u2 = fabs(rmean) * t;
      if (u1 < u2) {
         // Stop refining estimate because desired accuracy has been reached.
         sum += rmean * len;
      } else {
